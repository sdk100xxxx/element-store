  ...
  return (
    <header className="sticky top-0 z-50 w-full bg-element-black/95 backdrop-blur-sm">
      ...
      >
        ...
        <div className="mobile-menu-scroll min-h-0 flex-1 overflow-y-auto px-4 py-3">
          ...
          <form action="/search" method="GET" className="relative mt-2 px-1">
            <input
              name="q"
              type="search"
              placeholder="Search..."
              className="w-full rounded-lg border border-element-gray-700 bg-element-gray-800 py-2.5 pl-9 pr-3 text-sm text-white placeholder-gray-500 focus:border-element-red focus:outline-none"
            />
            <svg className="pointer-events-none absolute left-3.5 top-1/2 h-4 w-4 -translate-y-1/2 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
            </svg>
          </form>
          <div className="mt-3 flex flex-col gap-1.5 border-t border-element-gray-800 pt-3">
            <span className="mb-0.5 text-[11px] font-semibold uppercase tracking-wider text-gray-500">Community & account</span>
            <a
              href={DISCORD_URL}
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center gap-2.5 rounded-lg bg-[#5865F2] px-3 py-2.5 text-sm font-medium text-white transition hover:bg-[#4752C4]"
            >
            ...
            {status === "loading" ? null : session ? (
              <>
                {["ADMIN", "PARTNER"].includes((session.user as { role?: string })?.role ?? "") && (
                  <Link href="/admin" className="rounded-lg bg-element-red px-3 py-2.5 text-center text-sm font-medium text-white transition hover:bg-element-red-dark">
                    ADMIN
                  </Link>
                )}
                <button onClick={() => signOut()} className="rounded-lg bg-element-gray-800 px-3 py-2.5 text-center text-sm font-medium text-white transition hover:bg-element-gray-700">
                  Sign Out
                </button>
              </>
            ) : (
              <>
                <Link href="/auth/signin" className="rounded-lg bg-element-red px-3 py-2.5 text-center text-sm font-medium text-white transition hover:bg-element-red-dark">
                  Sign In
                </Link>
                <Link href="/auth/signup" className="rounded-lg border border-element-gray-600 bg-element-gray-800 px-3 py-2.5 text-center text-sm font-medium text-white transition hover:bg-element-gray-700">
                  Sign Up
                </Link>
              </>
            )}
