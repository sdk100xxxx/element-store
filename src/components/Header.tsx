...
export function Header() {
  ...
  return (
    <header className="sticky top-0 z-50 w-full bg-element-black/95 backdrop-blur-sm">
      ...
      {/* Mobile: compact popup – all options in one small panel, no scroll */}
      {menuOpen && (
        <>
          <div
          ...
          />
          <div
            className="fixed left-1/2 top-[4rem] z-50 w-[min(calc(100vw-2rem),20rem)] -translate-x-1/2 rounded-xl border border-element-gray-800 bg-element-black p-3 shadow-2xl md:hidden"
            role="dialog"
            aria-modal="true"
            aria-label="Menu"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex items-center justify-between border-b border-element-gray-800 pb-2 mb-2">
              <span className="text-xs font-semibold uppercase tracking-wider text-gray-500">Menu</span>
              <button
                type="button"
                onClick={() => setMenuOpen(false)}
                className="flex h-8 w-8 items-center justify-center rounded-lg text-gray-400 transition hover:bg-element-gray-800 hover:text-white"
            ...
            <div className="flex flex-col gap-0.5">
              ...
              <form action="/search" method="GET" className="relative mt-1">
                <input
                  name="q"
                  type="search"
                  placeholder="Search..."
                  className="w-full rounded-lg border border-element-gray-700 bg-element-gray-800 py-2 pl-8 pr-2.5 text-sm text-white placeholder-gray-500 focus:border-element-red focus:outline-none"
                />
              ...
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="mt-1 flex items-center gap-2 rounded-lg bg-[#5865F2] px-2.5 py-2 text-sm font-medium text-white transition hover:bg-[#4752C4]"
              >
              ...
              {status === "loading" ? null : session ? (
                <div className="mt-1 flex gap-1.5 border-t border-element-gray-800 pt-2">
                  {["ADMIN", "PARTNER"].includes((session.user as { role?: string })?.role ?? "") && (
                    <Link href="/admin" className="flex-1 rounded-lg bg-element-red px-2.5 py-2 text-center text-sm font-medium text-white transition hover:bg-element-red-dark">
                      Admin
                    </Link>
                  )}
                  <button type="button" onClick={() => signOut()} className="flex-1 rounded-lg bg-element-gray-800 px-2.5 py-2 text-sm font-medium text-white transition hover:bg-element-gray-700">
                    Sign Out
                  </button>
                </div>
              ) : (
                <div className="mt-1 flex gap-1.5 border-t border-element-gray-800 pt-2">
                  <Link href="/auth/signin" className="flex-1 rounded-lg bg-element-red px-2.5 py-2 text-center text-sm font-medium text-white transition hover:bg-element-red-dark">
                    Sign In
                  </Link>
                  <Link href="/auth/signup" className="flex-1 rounded-lg border border-element-gray-600 bg-element-gray-800 px-2.5 py-2 text-center text-sm font-medium text-white transition hover:bg-element-gray-700">
                    Sign Up
                  </Link>
                </div>
              )}
