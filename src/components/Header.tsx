...
export function Header() {
  ...
  return (
    <header className="sticky top-0 z-50 w-full bg-element-black/95 backdrop-blur-sm">
      {/* Top bar: logo, center Discord (desktop only), right: menu button (mobile) or Discord + auth (desktop) */}
      <div className="flex min-h-[3.25rem] items-center justify-between gap-3 border-b border-element-gray-800 px-4 py-2.5 sm:px-6">
        ...
        {/* Center: Join Discord - desktop only */}
        <a
          href={DISCORD_URL}
          target="_blank"
          rel="noopener noreferrer"
          className="absolute left-1/2 top-1/2 hidden -translate-x-1/2 -translate-y-1/2 items-center gap-2 rounded px-3 py-1.5 text-sm font-medium text-element-red transition hover:bg-element-red hover:text-white md:flex"
        >
      ...
      <div
        className={`fixed top-0 right-0 z-50 h-full w-full max-w-sm border-l border-element-gray-800 bg-element-black shadow-xl transition-transform duration-200 ease-out md:hidden ${
